...
// const User_collection = require("../Models/UserSginupSchema");
const User_Regester_collection = require("../Models/UserRegeseterSchema");
...
const user_regester = async (req, res) => {
    try {
        ...
        const create_user = User_Regester_collection(body)
        const Data_save_in_db = await create_user.save(create_user)
        // delete Data_save_in_db.password
        delete Data_save_in_db.password
...
const user_login = async (req, res) => {
    try {
        ...
        const find_user = await User_Regester_collection.findOne({ 
            email: body.email
         })
       ...
       res.status(200).json({user_data:user_data, compare_password,create_token})
...
//         const req_cookie_token =  req.headers.authorization;
...
//         }
//         const auth_token = req_cookie_token.split(' ')[1]
 
//         // when user token is available then verify token 
//         const verify_token = jwt.verify(auth_token, "@asdasd");
//         if(!verify_token){
//             return res.status(402).json({
//                 success: false,
//                 message: 'Login required!'
...
module.exports = { user_regester,user_login}